...
def read_issue_parsed_status_data():
    os.chdir('c:\\Users\\t1nipun\\Desktop\\PCMR\\human-robot\\Data_Analysis')
    df = pd.read_csv('issues_parsed_status.csv', encoding='utf-8-sig', keep_default_na = False)
    return df

def populate_status_issue_parsed_table():
    ...
    data = read_issue_parsed_status_data()
    with engine.connect() as conn:
        for issue in data.itertuples():
            conn.execute(update_bert_status_query, (issue.predicted_status_label, issue.unresolved_probability, issue.resolved_probability, issue.tableId, issue.rowIndex, issue.rowCounter))
        conn.execute('UPDATE issues_parsed SET status_confidence_issue_parsed = abs(unres_prob-res_prob)*100')
    print('Done')
...
if __name__ == "__main__":
    ...
    #read_issue_parsed_status_data()
    #populate_status_issue_parsed_table()